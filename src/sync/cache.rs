...
impl<K, V, S> Cache<K, V, S> {
    ...
    ///
    /// ```rust
    /// use moka::{snapshot::SnapshotAccuracy, sync::Cache};
    ///
    /// let cache = Cache::new(100);
    /// cache.insert('n', "Netherland Dwarf");
    ///
    /// // The insert above has not been processed by the cache's maintenance
    /// // yet, so the snapshot reports unmaintained estimates.
...
mod tests {
    ...
    mod snapshot {
        ...
        #[test]
        fn unmaintained_until_first_maintenance() {
        ...
        #[test]
        fn self_consistent_after_weighted_eviction() {
            let mut cache = Cache::builder()
                .max_capacity(10)
                .weigher(|_k, _v| 5u32)
                .eviction_policy(EvictionPolicy::lru())
        ...
        #[test]
        fn self_consistent_after_expiration() {
            let (clock, mock) = Clock::mock();
            let mut cache = Cache::builder()
                .max_capacity(100)
        ...
        #[test]
        fn self_consistent_after_explicit_invalidation() {
            let mut cache = Cache::builder().max_capacity(100).build();
            cache.reconfigure_for_testing();
            let cache = cache;
        ...
        #[test]
        fn generation_shared_across_clones() {
            let mut cache = Cache::builder().max_capacity(100).build();
            cache.reconfigure_for_testing();
            let cache = cache;
        ...
        #[test]
        fn isolated_from_listener_panic() {
            let listener = |_k, _v, _cause| panic!("listener failed");
            let mut cache = Cache::builder()
                .max_capacity(1)
                .eviction_listener(listener)
                .build();
        ...
        #[test]
        fn self_consistent_under_concurrency() {
            let cache = Cache::builder()
                .max_capacity(1_000)
                .weigher(|_k, _v| 2u32)
                .build();
